package vectordb;

import java.util.ArrayList;
import java.util.List;
import java.util.PriorityQueue;

public class KDTree {
    private static class KDNode {
        VectorItem item;
        KDNode left, right;
        KDNode(VectorItem v) { this.item = v; }
    }

    private KDNode root;
    private int dims;

    public KDTree(int dims) {
        this.dims = dims;
    }

    public void insert(VectorItem v) {
        root = ins(root, v, 0);
    }

    private KDNode ins(KDNode n, VectorItem v, int d) {
        if (n == null) return new KDNode(v);
        int ax = d % dims;
        if (v.emb[ax] < n.item.emb[ax]) n.left = ins(n.left, v, d + 1);
        else n.right = ins(n.right, v, d + 1);
        return n;
    }

    public List<BruteForce.Hit> knn(float[] q, int k, Distance dist) {
        PriorityQueue<BruteForce.Hit> heap = new PriorityQueue<>(k + 1, (a, b) -> Float.compare(b.dist, a.dist));
        knn(root, q, k, 0, dist, heap);
        
        List<BruteForce.Hit> res = new ArrayList<>();
        while (!heap.isEmpty()) res.add(heap.poll());
        java.util.Collections.reverse(res);
        return res;
    }

    private void knn(KDNode n, float[] q, int k, int d, Distance dist, PriorityQueue<BruteForce.Hit> heap) {
        if (n == null) return;
        float dn = dist.compute(q, n.item.emb);
        if (heap.size() < k || dn < heap.peek().dist) {
            heap.offer(new BruteForce.Hit(dn, n.item.id));
            if (heap.size() > k) heap.poll();
        }
        int ax = d % dims;
        float diff = q[ax] - n.item.emb[ax];
        KDNode closer = diff < 0 ? n.left : n.right;
        KDNode farther = diff < 0 ? n.right : n.left;
        
        knn(closer, q, k, d + 1, dist, heap);
        if (heap.size() < k || Math.abs(diff) < heap.peek().dist) {
            knn(farther, q, k, d + 1, dist, heap);
        }
    }

    public void rebuild(List<VectorItem> items) {
        root = null;
        for (VectorItem v : items) insert(v);
    }
}
